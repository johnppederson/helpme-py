...
#define HELPME_ERF_VAL static_cast<Real>(ERF_SCREEN_VAL)

template <typename Real>
static inline Real screened_erf(Real x) {
    return (x > HELPME_ERF_VAL) ? Real(1.0) : std::erf(x);
}

template <typename Real>
static inline Real screened_erfc(Real x) {
    return Real(1.0) - screened_erf<Real>(x);
}
...
struct incompleteGammaRecursion<Real, 1, false> {
    static Real compute(Real x) { return HELPME_SQRTPI * screened_erfc<Real>(std::sqrt(x)); }
};
