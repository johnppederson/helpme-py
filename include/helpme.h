...
class PMEInstance {
   ...
   protected:
     ...
     * (minus) the field, etc.  A negative value indicates that only the derivative with order |derivativeLevel|
     ...
     * nAtoms x nD, where nD is the derivative level requested.  See the details for the parameters argument for
...
#pragma omp parallel num_threads(nThreads_)
     ...
     */
    helpme::Matrix<Real> minimumImageDeltaR(const typename helpme::Matrix<Real>::sliceIterator &positionI,
        ...
        return helpme::Matrix<Real>({{dxR - sxR, dyR - syR, dzR - szR}});
    }
     ...
     * ShapeMatrix: enforce a symmetric representation of the lattice vectors [c.f. S. Nos${'e}$ and M. L. Klein,
...
#pragma omp for
     ...
     */
    Real computeEDir(const PairList &pairList, int parameterAngMom, const RealMat &parameters,
        ...
        for (int pair = 0; pair < nPair; ++pair) {
            ...
            auto deltaR = minimumImageDeltaR(coordsI, coordsJ);
     ...
     * \return the direct space energy.
     */
    Real computeEFDir(const PairList &pairList, int parameterAngMom, const RealMat &parameters,
     ...
     * \return the direct space energy.
     */
    Real computeEFVDir(const PairList &pairList, int parameterAngMom, const RealMat &parameters,
     ...
     */
    Real computeEAdj(const PairList &pairList, int parameterAngMom, const RealMat &parameters,
                     const RealMat &coordinates) {
        if (parameterAngMom) throw std::runtime_error("Multipole self terms have not been coded yet.");
        sanityChecks(parameterAngMom, parameters, coordinates);
     ...
     * \return the adjusted energy.
     */
    Real computeEFAdj(const PairList &pairList, int parameterAngMom, const RealMat &parameters,
                      const RealMat &coordinates, RealMat &forces) {
        if (parameterAngMom) throw std::runtime_error("Multipole self terms have not been coded yet.");
        sanityChecks(parameterAngMom, parameters, coordinates);
     ...
     * \return the adjusted energy.
     */
    Real computeEFVAdj(const PairList &pairList, int parameterAngMom, const RealMat &parameters,
                       const RealMat &coordinates, RealMat &forces, RealMat &virial) {
        if (parameterAngMom) throw std::runtime_error("Multipole self terms have not been coded yet.");
        sanityChecks(parameterAngMom, parameters, coordinates);
...
#pragma omp parallel for num_threads(nThreads_)
        for (size_t i = 0; i < nAtoms; ++i) {
            ...
            for (size_t j = 0; j < nAtoms; ++j) {
                ...
                const auto &coordsJ = coordinates.row(j);
                auto deltaR = minimumImageDeltaR(coordsI, coordsJ);
                // Real rSquared = RIJ[0][0] * RIJ[0][0] + RIJ[0][1] * RIJ[0][1] + RIJ[0][2] * RIJ[0][2];
                Real rSquared = deltaR.dot(deltaR);
...
#pragma omp parallel for num_threads(nThreads_)
    ...
    /*!
     * \brief Computes the adjusted real space potential from the atoms at the grid points provided.
     * \param parameterAngMom the angular momentum of the parameters (0 for charges, C6 coefficients, 2 for
     * quadrupoles, etc.).  A negative value indicates that only the shell with |parameterAngMom| is to be considered,
     * e.g. a value of -2 specifies that only quadrupoles (and not dipoles or charges) will be provided; the input
     * matrix should have dimensions corresponding only to the number of terms in this shell.
     ...
     * coordinates.
     * \param potential the array holding the potential.  This is a matrix of dimensions
     * nAtoms x 1. N.B. this array is incremented with the potential, not
     * assigned, so take care to zero it first if only the current results are desired.
     * \param minimumImage whether or not to use the minimum image convection when calculated the potential.
     */
    void computePAdj(int parameterAngMom, const RealMat &parameters, const RealMat &coordinates,
                     const RealMat &gridPoints, RealMat &potential, bool &minimumImage) {
        if (parameterAngMom) throw std::runtime_error("Multipole adjusted potential terms have not been coded yet.");
        if (rPower_ != 1) throw std::runtime_error("Non-coulombic adjusted potential terms have not been coded yet.");
        sanityChecks(parameterAngMom, parameters, coordinates);
        size_t nAtoms = coordinates.nRows();
        size_t nGridPoints = gridPoints.nRows();
        Real kappaSquared = kappa_ * kappa_;
        Real slfPrefac = 2 * slfEFxn_(parameterAngMom, kappa_, scaleFactor_);
        Real shift = static_cast<Real>(minimumImage);
        const Real *__restrict__ coordPtr = coordinates.begin();
        const Real *__restrict__ gridPtr = gridPoints.begin();
        const Real *__restrict__ paramPtr = parameters.begin();
#pragma omp parallel for num_threads(nThreads_) schedule(static)
        for (size_t i = 0; i < nGridPoints; ++i) {
            const Real gx = gridPtr[3 * i];
            const Real gy = gridPtr[3 * i + 1];
            const Real gz = gridPtr[3 * i + 2];
            Real phi = 0;
            for (size_t j = 0; j < nAtoms; ++j) {
                Real dx = gx - coordPtr[3 * j];
                Real dy = gy - coordPtr[3 * j + 1];
                Real dz = gz - coordPtr[3 * j + 2];
                if (minimumImage) {
                    // Convert to fractional coordinate (S) space
                    Real dxS = recVecs_[0][0] * dx + recVecs_[0][1] * dy + recVecs_[0][2] * dz;
                    Real dyS = recVecs_[1][0] * dx + recVecs_[1][1] * dy + recVecs_[1][2] * dz;
                    Real dzS = recVecs_[2][0] * dx + recVecs_[2][1] * dy + recVecs_[2][2] * dz;
                    // Apply translations in fractional coordinates to find the shift vectors
                    Real sxS = std::floor(dxS + 0.5f);
                    Real syS = std::floor(dyS + 0.5f);
                    Real szS = std::floor(dzS + 0.5f);
                    // Convert fractional coordinate shifts to real space
                    dx -= boxVecs_[0][0] * sxS + boxVecs_[0][1] * syS + boxVecs_[0][2] * szS;
                    dy -= boxVecs_[1][0] * sxS + boxVecs_[1][1] * syS + boxVecs_[1][2] * szS;
                    dz -= boxVecs_[2][0] * sxS + boxVecs_[2][1] * syS + boxVecs_[2][2] * szS;
                }
                Real rSquared = dx * dx + dy * dy + dz * dz;
                Real prefac = (rSquared <= 1e-6) ? slfPrefac : scaleFactor_ * adjEImpl<1>(rSquared, kappaSquared);
                phi += prefac * paramPtr[j];
            }
            potential(i, 0) += phi;
        }
    }
    ...
    /*!
     * \brief Computes the adjusted real space potential and its first derivative from the atoms at the grid points
     * provided.
     * \param parameterAngMom the angular momentum of the parameters (0 for charges, C6 coefficients, 2 for
     * quadrupoles, etc.).  A negative value indicates that only the shell with |parameterAngMom| is to be considered,
     * e.g. a value of -2 specifies that only quadrupoles (and not dipoles or charges) will be provided; the input
     ...
     * \param minimumImage whether or not to use the minimum image convection when calculated the potential.
     */
    void computePDPAdj(int parameterAngMom, const RealMat &parameters, const RealMat &coordinates,
                       const RealMat &gridPoints, RealMat &potential, bool &minimumImage) {
        if (parameterAngMom) throw std::runtime_error("Multipole self terms have not been coded yet.");
        if (rPower_ != 1) throw std::runtime_error("Non-coulombic adjusted potential terms have not been coded yet.");
        sanityChecks(parameterAngMom, parameters, coordinates);
        ...
        size_t nGridPoints = gridPoints.nRows();
        Real kappaSquared = kappa_ * kappa_;
        Real eSlfPrefac = 2 * slfEFxn_(parameterAngMom, kappa_, scaleFactor_);
        Real gSlfPrefac = 0.0;
        const Real *__restrict__ coordPtr = coordinates.begin();
        const Real *__restrict__ gridPtr = gridPoints.begin();
        const Real *__restrict__ paramPtr = parameters.begin();
#pragma omp parallel for num_threads(nThreads_) schedule(static)
        for (size_t i = 0; i < nGridPoints; ++i) {
            ...
            const Real gy = gridPtr[3 * i + 1];
            const Real gz = gridPtr[3 * i + 2];
            Real phi = 0, phiX = 0, phiY = 0, phiZ = 0;
            for (size_t j = 0; j < nAtoms; ++j) {
                Real dx = gx - coordPtr[3 * j];
                Real dy = gy - coordPtr[3 * j + 1];
                Real dz = gz - coordPtr[3 * j + 2];
                ...
                Real rSquared = dx * dx + dy * dy + dz * dz;
                auto [eKernel, gKernel] = adjEFImpl<1>(rSquared, kappa_, kappaSquared);
                Real ePrefac = (rSquared <= 1e-6) ? eSlfPrefac : scaleFactor_ * eKernel;
                Real gPrefac = (rSquared <= 1e-6) ? gSlfPrefac : scaleFactor_ * gKernel;
                Real qJ = paramPtr[j];
                phi += ePrefac * qJ;
                phiX += gPrefac * dx * qJ;
                phiY += gPrefac * dy * qJ;
     ...
     */
    Real computeEAll(const PairList &includedList, const PairList &excludedList, int parameterAngMom,
     ...
     * \return the full PME energy.
     */
    Real computeEFAll(const PairList &includedList, const PairList &excludedList, int parameterAngMom,
     ...
     * \return the full PME energy.
     */
    Real computeEFVAll(const PairList &includedList, const PairList &excludedList, int parameterAngMom,
