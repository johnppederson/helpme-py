...
int main(int argc, char *argv[]) {
    ...
    if (numNodes == 1) {
        ...
        pme->setupCompressedParallel(rPower, beta, splineOrder, gridA, gridB, gridC, maxKA, maxKB, maxKC, scaleFactor,
                                     numThreads, MPI_COMM_WORLD, helpme::NodeOrder::ZYX, numNodesX, numNodesY,
                                     numNodesZ);
#endif
    }
    pme->setLatticeVectors(boxDimX, boxDimY, boxDimZ, 90, 90, 90, helpme::LatticeType::XAligned);
    ...
    pmeSerial->setLatticeVectors(boxDimX, boxDimY, boxDimZ, 90, 90, 90, helpme::LatticeType::XAligned);
    ...
    if (myRank == 0) {
        ...
        std::shuffle(atomList.begin(), atomList.end(), g);
    }
#if HAVE_MPI == 1
