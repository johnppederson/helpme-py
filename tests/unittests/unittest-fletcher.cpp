...
TEST_CASE("test the fletcher checksums.") {
    ...
    SECTION("errors are thrown for tiny data") {
        REQUIRE_THROWS_WITH(
            helpme::compute_fletcher_checksum16(expr, 0),
            "Not enough data to compute a fletcher16 checksum"
        );
        REQUIRE_THROWS_WITH(
            helpme::compute_fletcher_checksum32(expr, 1),
            "Not enough data to compute a fletcher32 checksum"
        );
        REQUIRE_THROWS_WITH(
            helpme::compute_fletcher_checksum64(expr, 3),
            "Not enough data to compute a fletcher64 checksum"
        );
    }
}
