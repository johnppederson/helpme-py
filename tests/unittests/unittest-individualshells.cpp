...
TEST_CASE("test reciprocal space computations using only partial shells") {
    ...
    SECTION("potential and field tests") {
        ...
        refpme->setLatticeVectors(A, B, C, 90, 90, 90, helpme::LatticeType::XAligned);
        ...
        SECTION("full potential and field from dipoles only") {
            ...
            pme->setLatticeVectors(A, B, C, 90, 90, 90, helpme::LatticeType::XAligned);
        ...
        SECTION("field only from dipoles only") {
            ...
            for (int atom = 0; atom < 6; ++atom) {
                REQUIRE(fieldOnly[atom][0] == Catch::Approx(refPotentialAndField[atom][1]).margin(TOL));
                REQUIRE(fieldOnly[atom][1] == Catch::Approx(refPotentialAndField[atom][2]).margin(TOL));
                REQUIRE(fieldOnly[atom][2] == Catch::Approx(refPotentialAndField[atom][3]).margin(TOL));
            }
        }
