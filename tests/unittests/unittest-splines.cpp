...
TEST_CASE("make sure B-Splines and their derivatives are created correctly.") {
    ...
    SECTION("double precision tests") {
        ...
        REQUIRE(firstDerivative[0] == Catch::Approx(-6.55066667e-03).margin(TOL));
        REQUIRE(firstDerivative[1] == Catch::Approx(-3.68264000e-01).margin(TOL));
        REQUIRE(firstDerivative[2] == Catch::Approx(-1.95904000e-01).margin(TOL));
        REQUIRE(firstDerivative[3] == Catch::Approx(5.22802667e-01).margin(TOL));
        REQUIRE(firstDerivative[4] == Catch::Approx(4.79160000e-02).margin(TOL));
