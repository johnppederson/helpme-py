...
std::tuple<Real, helpme::Matrix<Real>, helpme::Matrix<Real>> dispersionKappaSweepHelper(Real kappa, CalcType type) {
    ...
    virial.setZero();
    std::vector<int> includedList;
    std::vector<int> excludedList;
    ...
    auto includedPairs = helpme::Matrix<int>(includedList.data(), includedList.size() / 2, 2);
    auto excludedPairs = helpme::Matrix<int>(excludedList.data(), excludedList.size() / 2, 2);
...
TEST_CASE(
    ...
    SECTION("EFV routine tests") {
        ...
        REQUIRE(energy25 == Catch::Approx(energy35).margin(TOL));
        ...
        REQUIRE(energy35 == Catch::Approx(energy45).margin(TOL));
