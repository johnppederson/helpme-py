...
TEST_CASE("check potential (and derivatives thereof) code.") {
    ...
    SECTION("potential tests up to field hessians") {
        ...
        pme.setLatticeVectors(20, 22, 25, 70, 85, 100, helpme::LatticeType::XAligned);
    ...
    SECTION("check consistency of energy and potential") {
        ...
        auto pairs = helpme::Matrix<int>(pairList.data(), pairList.size() / 2, 2);
        helpme::Matrix<int> empty;
        helpme::Matrix<double> coords(
            {{2.0, 2.0, 2.0}, {2.5, 2.0, 3.0}, {1.5, 2.0, 3.0}, {0.0, 0.0, 0.0}, {0.5, 0.0, 1.0}, {-0.5, 0.0, 1.0}});
        helpme::Matrix<double> charges({-0.834, 0.417, 0.417, -0.834, 0.417, 0.417});
        ...
        REQUIRE(Epot == Catch::Approx(Eall).margin(1e-7));
    }
    ...
    SECTION("charge only potential at atomic sites") {
        ...
        pme3.setLatticeVectors(34, 33, 35, 80, 85, 100, helpme::LatticeType::XAligned);
        pme4.setLatticeVectors(34, 33, 35, 80, 85, 100, helpme::LatticeType::XAligned);
        ...
        REQUIRE_THROWS_WITH(
            pme3.computePAtAtomicSites(charges, coords, potential3, 25),
            "The cutoff used must be less than half of the minimum of three box widths"
        );
    }
    ...
    SECTION("charge only potential at atomic sites, orthorhombic, compressed") {
        ...
        pme3.setLatticeVectors(34, 33, 35, 90, 90, 90, helpme::LatticeType::XAligned);
        pme4.setLatticeVectors(34, 33, 35, 90, 90, 90, helpme::LatticeType::XAligned);
        helpme::Matrix<double> potential3(9, 1);
        helpme::Matrix<double> potential4(9, 1);
        pme3.computePAtAtomicSites(charges, coords, potential3, 15);
        pme4.computePAtAtomicSites(charges, coords, potential4, 15);
