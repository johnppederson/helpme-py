...
TEST_CASE("Full run with a small toy system, comprising two water molecules with multipoles.") {
    ...
    SECTION("cubic box tests") {
        ...
        SECTION("double precision tests") {
            ...
            pmeD->setLatticeVectors(A, B, C, 90, 90, 90, helpme::LatticeType::XAligned);
            double energy = pmeD->computeEFVRecIsotropicInducedDipoles(
                0, paramsD, inducedDipolesD, helpme::PolarizationType::Mutual, coordsD, forcesD, virialD);
            REQUIRE(refEnergy == Catch::Approx(energy).margin(TOL));
        ...
        SECTION("single precision tests") {
            ...
            pmeF->setLatticeVectors(A, B, C, 90, 90, 90, helpme::LatticeType::XAligned);
            double energy = pmeF->computeEFVRecIsotropicInducedDipoles(
                0, paramsD.cast<float>(), inducedDipolesD.cast<float>(), helpme::PolarizationType::Mutual,
    ...
    SECTION("triclinic box tests") {
        ...
        SECTION("double precision tests") {
            ...
            auto pmeD = std::unique_ptr<PMEInstanceD>(new PMEInstanceD);
            pmeD->setup(1, 0.3, splineOrder, nfftx, nffty, nfftz, ccelec, numThreads);
            pmeD->setLatticeVectors(A, B, C, 75, 80, 85, helpme::LatticeType::XAligned);
            double energy = pmeD->computeEFVRecIsotropicInducedDipoles(
                0, paramsD, inducedDipolesD, helpme::PolarizationType::Mutual, coordsD, forcesD, virialD);
            REQUIRE(refEnergy == Catch::Approx(energy).margin(TOL));
            REQUIRE(refForcesD.almostEquals(forcesD, TOL));
        ...
        SECTION("single precision tests") {
            ...
            auto pmeF = std::unique_ptr<PMEInstanceF>(new PMEInstanceF);
            pmeF->setup(1, 0.3, splineOrder, nfftx, nffty, nfftz, ccelec, numThreads);
            pmeF->setLatticeVectors(A, B, C, 75, 80, 85, helpme::LatticeType::XAligned);
            double energy = pmeF->computeEFVRecIsotropicInducedDipoles(
                0, paramsD.cast<float>(), inducedDipolesD.cast<float>(), helpme::PolarizationType::Mutual,
                coordsD.cast<float>(), forcesF, virialF);
            REQUIRE(refEnergy == Catch::Approx(energy).margin(TOL));
